const PLANT_ID_ENDPOINT = import.meta.env.VITE_PLANT_ID_ENDPOINT || 'https://plant.id/api/v3/identification';
...
export const identifyPlant = async (imageFile) => {
  ...
  const response = await fetch(`${PLANT_ID_ENDPOINT}?${params.toString()}`, {
    method: 'POST',
    headers: {
      'Api-Key': apiKey,
